...
InterruptIn wave_button(D2);

InterruptIn rise_freq(D3);
InterruptIn low_freq(D4);

BusOut signal(D6, D7, D8, D9, D10, D11, D12, D13);

// Tempo de espera entre cada ponto
float period = 0.00005;
// Índice atual da onda
int wave = 0;
// Look up table dos valores de seno de 0 a 90 normalizados
...
// Função que muda a frequencia das ondas
void change_freq()
{
    if (rise_freq == 1) {
        period = period / 1.2;
...
// Função que muda o tipo de onda que o DAC está gerando
void change_wave()
{
    if (wave < 3) {
        wave++;
